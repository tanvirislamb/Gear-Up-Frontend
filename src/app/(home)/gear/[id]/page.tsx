    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-7 space-y-8">
          {/* Hero Image */}
          <div className="relative aspect-[16/10] w-full rounded-3xl overflow-hidden bg-slate-900 border border-slate-800 shadow-2xl">
            {hasImage ? (
              <Image
                src={gear.image!}
                alt={gear.name}
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 sm:p-8 space-y-4">
            ...
            <p className="text-sm text-slate-300 leading-relaxed">{gear.description}</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-4 border-t border-slate-800/60 text-xs">
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                ...
                <span className="text-slate-200 font-semibold mt-0.5 block">Inspected / Verified</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="text-xs text-slate-400 font-medium bg-slate-950 px-4 py-2 rounded-xl border border-slate-800">
              ⭐ Verified Provider
            </div>
          </div>
          ...
          <div className="space-y-6 pt-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              ...
              <span className="text-xs font-semibold text-slate-400">{gear.reviews?.length || 0} ratings</span>
            </div>
              ...
              <div className="space-y-4">
                {gear.reviews.map((rev, idx) => (
                  <div key={rev.id || idx} className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-5 space-y-2">
                    ...
                    {rev.comment && (
                      <p className="text-xs text-slate-300 italic pl-10 leading-relaxed">&quot;{rev.comment}&quot;</p>
                    )}
                  </div>
                ))}
              </div>
